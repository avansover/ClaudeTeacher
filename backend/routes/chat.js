...
async function buildWorkPageContext(studentId) {
  ...
  const answerStr = ex.answer
    ? `\nExpected answer (ground truth, computed when this page was created — trust this over your own live math): "${ex.answer}". When judging whether her final answer is correct, compare it to THIS value, not to arithmetic you re-derive in this conversation. Accept any equivalent form — spelled-out numbers, a different but mathematically equal fraction or expression, Hebrew or English wording — the underlying value must match, not the exact text. If it doesn't match this expected answer, she's wrong, no matter how confident your own recalculation feels.`
    : `\nNo stored answer for this exercise (it predates this feature) — grade it yourself as before, carefully.`;

  return {
    context: `\n\nWORK PAGE (${modeInstr})\nTitle: "${page.title}" | Subject: ${page.subject} | Progress: ${page.done}/${page.total} (${pct}%).${dueStr}\nCurrent exercise (id=${ex.id}, difficulty=${ex.difficulty}): "${ex.description}"${answerStr}\nYou must present THIS EXACT exercise text to her, word for word — never invent a new, similar, or "next" problem of your own, even if it feels like a natural continuation of the pattern. mark_exercise only works for id=${ex.id}, which is tied to this exact text; if you present a different problem instead, there is no exercise to mark and her progress will not update, with no way to fix it from inside the conversation. If you realize you already presented something other than this exact exercise, stop, tell her honestly what happened, and switch to presenting this exact exercise before continuing. Only call mark_exercise once she has actually reached the correct final answer to THIS exercise in this conversation. If she wants to skip it or stop without solving it, call status="skipped" — never "solved". Total exercises remaining on this page (including this one): ${page.total - page.done}.`,
